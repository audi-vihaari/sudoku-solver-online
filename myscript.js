...
function issafe(row,col,num)
{
  ...
  for(var c=0;c<9;c++)
  {
    table.eq(row).find('td').eq(c).find('input').css("background-color","ffffff");
  ...
  for(var r=0;r<9;r++)
  {
    table.eq(r).find('td').eq(col).find('input').css("background-color","ffffff");
  ...
  for(var r=0;r<3;r++)
  {
    for(var c=0;c<3;c++)
    {
      table.eq(r+startrow).find('td').eq(c+startcol).find('input').css("background-color","ffffff");
...
function changecolor()
{
  for(var r=0;r<9;r++)
  {
    for(var c=0;c<9;c++)
    {
      table.eq(r).find('td').eq(c).find('input').css("background-color","#ffffff");
    }
  }
}
...
{
  $(".refreshclass").css("background-color","#ffffff");
